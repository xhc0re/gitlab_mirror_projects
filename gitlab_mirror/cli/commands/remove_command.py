...
Warning:
    ...
    if not args.pattern and not args.remove_failed and not args.all:
        logger.error("No selection criteria provided. Please specify at least one of: --pattern, --remove-failed, or --all")
    ...
    try:
        if args.dry_run:
            print("DRY RUN MODE: No mirrors will be removed. Run without --dry-run to remove mirrors.")
        ...
        if args.all:
            ...
            confirmation = input("Are you ABSOLUTELY sure? This cannot be undone. Type 'YES' (all capitals) to confirm: ")
            if confirmation != "YES":
            ...
            confirmation = input("Are you sure you want to remove these mirrors? This cannot be undone. (yes/no): ")
