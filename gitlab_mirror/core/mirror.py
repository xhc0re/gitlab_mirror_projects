...
from gitlab.exceptions import (
    GitlabGetError, GitlabCreateError,
    GitlabListError, GitlabHttpError
)
import pandas as pd

from gitlab_mirror.core.config import GitLabConfig, MirrorConfig
from gitlab_mirror.core.exceptions import ConfigError, ApiError
...
class MirrorService:
    ...
    def load_project_mappings(self) -> List[ProjectMapping]:
        ...
        try:
            ...
            raise ConfigError(f"Failed to load project mappings from {self.config.projects_file}") from e
    ...
    def setup_push_mirror(self, source_project, mirror_url: str) -> None:
        ...
        try:
            ...
            raise ApiError(f"Failed to list mirrors for {source_project.path_with_namespace}") from e
        except GitlabCreateError as e:
            logger.error("Failed to create mirror: %s", e)
            raise ApiError(f"Failed to create mirror for {source_project.path_with_namespace}") from e
    ...
    def trigger_mirror_sync(self, project_id: int) -> None:
        ...
        try:
            ...
            logger.error("Unexpected error in trigger_mirror_sync for project %s: %s", project_id, e)
    ...
    def mirror_project(self, mapping: ProjectMapping) -> bool:
        ...
        try:
            ...
            target_path = f"{target_group_path}/{project_name}" if target_group_path else project_name
            ...
            try:
                ...
                if not group_id:
                    logger.error("Cannot create project without a group. Target group path is empty.")
                    return False
            ...
            self.errors.append({
                ...
                'target': mapping.target_path if 'mapping_target_path' in locals() else mapping.target_path,
                'error_type': 'ApiError',
                'message': str(e)
            })
            return False
