...
def update_mirror_auth(mirror, new_token: str, old_domain: str = None, new_domain: str = None) -> bool:
...
def process_project_mirrors(
    ...
    try:
        ...
        for mirror in mirrors:
            ...
            if (pattern and re.search(pattern, mirror_url, re.IGNORECASE)) or (update_failed and is_mirror_failing(mirror)):
                logger.info("Processing mirror in project %s: %s", project.path_with_namespace, mirror_url)
                if not dry_run:
                    if update_mirror_auth(mirror, new_token, old_domain, new_domain):
                        ...
                        try:
                            mirror.delete()
                            mirrors_removed += 1
                            logger.info("Removed failed mirror from project %s", project.path_with_namespace)
                        except GitlabError as e:
                            logger.error("Failed to remove mirror from project %s: %s", project.path_with_namespace, str(e))
                else:
                    logger.info("[DRY RUN] Would update mirror in project %s", project.path_with_namespace)
                    mirrors_updated += 1
    except GitlabError as e:
        logger.error("Error processing mirrors for project %s: %s", project.path_with_namespace, str(e))
...
def update_mirrors(
        ...
        for project in projects:
            ...
            try:
                ...
                updated, removed = process_project_mirrors(
                    project_with_mirrors, new_mirror_token, pattern, update_failed, old_domain, new_domain, dry_run
                )
    ...
    if failed_projects:
        ...
        logger.info("Exported %d failed projects to 05-update-failed-projects.csv", len(failed_projects))
