    ...
    for project in projects:
        ...
        try:
            ...
            for mirror in mirrors:
                ...
                if remove_all:
                    should_remove = True
                    logger.info("%s mirror from project %s (remove_all flag)",
                ...
                # If a pattern is provided and the mirror URL matches the pattern
                    ...
                    should_remove = True
                    logger.info("Found mirror matching pattern in project %s: %s",
                                project.path_with_namespace, mirror_url)
                
                # If remove_failed is True and the mirror has an issue
                elif remove_failed and (not mirror.enabled or (hasattr(mirror, 'last_error') and mirror.last_error)):
                    should_remove = True
                    error_info = mirror.last_error if hasattr(mirror, 'last_error') else "disabled"
                    logger.info("Found failed mirror in project %s: %s",
                                project.path_with_namespace, error_info)
                ...
                if should_remove:
                    ...
                    if dry_run:
                        ...
                        try:
                            ...
                            project_mirrors_removed += 1
                            mirrors_removed += 1
                            logger.info("Successfully removed mirror from %s",
                                        project.path_with_namespace)
                        except Exception as e:  # pylint: disable=broad-exception-caught
                            logger.error("Error removing mirror from %s: %s",
                                        project.path_with_namespace, str(e))
                            error_count += 1
            
            if project_mirrors_removed > 0:
                if not dry_run:
                    logger.info("Removed %d mirrors from %s",
                                project_mirrors_removed, project.path_with_namespace)
            
            if project_matches:
                matching_projects += 1
                        
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error("Error processing project %s: %s",
                        project.path_with_namespace, str(e))
